...
import { AvailabilitiesResponse, EventsResponse, UserResponse } from "../../types"
...
export default class ActiefAPI {
   ...
   */
  public static async login(username: string, password: string): Promise<string | undefined> {
    const url =
   ...
   * @param cookies
   */
  public static async AuthorizeClient(
    cookies: string
  ): Promise<{ code: string; state: string; session_state: string; code_verifier: string }> {
   ...
   * @param code The code received from the authorization of the client
   * @param code_verifier The code verifier used with the code
   */
  ...
  public static async getNewAccessTokenWithRefreshToken(refresh_token: string): Promise<string> {
    const url = "https://login.actief.be/connect/token"
  ...
  public static async getCurrentUser(token: string): Promise<UserResponse> {
  ...
  public static async getCalenderEvents(
    token: string,
    from: Date,
    to: Date
  ): Promise<EventsResponse> {
    const candidateId = await this.getCurrentCandidate(token)
    const url = `https://core-client-api.actief.be/portal/v1/candidate/${candidateId}/calender-overview-events?from=${from.toLocaleDateString(
      "en-US"
    )}&until=${to.toLocaleDateString("en-US")}`
    const res = await fetch(url, {
      method: "GET",
      headers: {
  ...
  public static async getAvailabilities(token: string): Promise<AvailabilitiesResponse[]> {
    const candidateId = await this.getCurrentCandidate(token)
  ...
  public static async getCurrentCandidate(token: string): Promise<number> {
  ...
  public static async registerNewAvailability(token: string, from: Date, to: Date) {
    const candidateId = await this.getCurrentCandidate(token)
    ...
    const data = {
      candidateCalendar: {
        ...
        from: `${from.getUTCFullYear()}-${(from.getUTCMonth() + 1)
          .toString()
          .padStart(2, "0")}-${from.getUTCDate().toString().padStart(2, "0")}`,
        hoursFrom: from.getUTCHours(),
        hoursUntil: to.getUTCHours(),
        minutesFrom: from.getUTCMinutes(),
        minutesUntil: to.getUTCMinutes(),
        until: `${to.getUTCFullYear()}-${(to.getUTCMonth() + 1).toString().padStart(2, "0")}-${to
          .getUTCDate()
          .toString()
          .padStart(2, "0")}`,
      },
